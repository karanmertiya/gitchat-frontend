...
export const api = {
  ...
  // NEW: Added parentMsgId so we can create the Anchor Commit
  ...
  merge: (sourceId: string, targetId: string, latestSourceMsgId: string, targetParentMsgId?: string | null) =>
    fetch(`${API_BASE}/merge`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ source_branch_id: sourceId, target_branch_id: targetId, latest_message_id_in_source: latestSourceMsgId, parent_message_id_in_target: targetParentMsgId }) }).then(res => res.json())
};