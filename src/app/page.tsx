...
export default function DialogTreeHome() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => {
        ...
        return [...filtered, { id: data.userMessageId, role: 'user', content: userPrompt }, { id: data.aiMessageId, role: 'ai', content: data.aiResponse }];
      });
    } catch (err: any) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      setInput(prev => prev + `\n\nI have attached a file named \`${file.name}\`:\n\`\`\`${ext}\n${content}\n\`\`\`\n`);
    };
  ...
  // ==========================================
  // RENDER: ENTERPRISE LOGIN SCREEN
  // ==========================================
  if (!session) {
    return (
        ...
        <div className="flex-1 flex flex-col justify-center items-center p-8 bg-zinc-950">
          <div className="w-full max-w-md">
            ...
            <div className="relative flex items-center py-4 mb-2">
              ...
              <span className="flex-shrink-0 mx-4 text-zinc-500 text-xs uppercase tracking-widest">Or continue with email</span>
              <div className="flex-grow border-t border-zinc-800"></div>
            </div>
            ...
            <form onSubmit={handleEmailAuth} className="space-y-4">
              ...
              <div>
                ...
                <input type="email" value={authEmail} onChange={e => setAuthEmail(e.target.value)} required className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500 transition-colors" placeholder="engineer@example.com" />
              </div>
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5 uppercase tracking-wider">Password</label>
                <input type="password" value={authPassword} onChange={e => setAuthPassword(e.target.value)} required className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-white focus:outline-none focus:border-indigo-500 transition-colors" placeholder="••••••••" />
              </div>
              <button type="submit" disabled={authLoading} className="w-full bg-white hover:bg-zinc-200 text-zinc-950 font-semibold py-3 rounded-xl transition-all disabled:opacity-50 flex justify-center items-center gap-2 mt-4">
                {authLoading ? <Loader2 size={18} className="animate-spin text-zinc-500" /> : null}
                {isSignUp ? 'Create Account' : 'Sign In'}
              </button>
            </form>

            <div className="mt-8 text-center">
              <button onClick={() => setIsSignUp(!isSignUp)} className="text-sm text-zinc-500 hover:text-indigo-400 transition-colors">
        ...
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold flex items-center gap-2"><DownloadCloud size={20} className="text-indigo-400"/> Import Shared Chat</h2>
              <button onClick={() => setImportModal(false)} className="text-zinc-500 hover:text-zinc-300"><X size={20}/></button>
            </div>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setImportModal(false)} className="px-4 py-2 text-sm text-zinc-400 hover:text-zinc-200">Cancel</button>
              <button onClick={handleImport} disabled={!importUrl} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-sm font-medium">Run Importer</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={submitFork} disabled={!forkModal.name.trim()} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white rounded-lg text-sm font-medium">Create Branch</button>
            </div>
          </div>
        </div>
      )}
      ...
      <aside className="w-64 border-r border-zinc-800 flex flex-col">
        <div className="p-4 flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <div className="bg-indigo-600 p-1.5 rounded-lg"><GitBranch size={18} className="text-white" /></div>
            <h1 className="font-bold text-md tracking-tight">DialogTree</h1>
          </div>
        </div>
          ...
          {branches.length === 0 ? (
            ...
            branches.map((b) => (
              <button 
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium transition-colors ${activeBranch?.id === b.id ? 'bg-zinc-800 text-indigo-400' : 'hover:bg-zinc-900 text-zinc-400 hover:text-zinc-200'}`}
              >
        ...
        <div className="p-4 border-t border-zinc-800 flex flex-col gap-2">
          <button onClick={() => setImportModal(true)} className="flex items-center justify-center gap-2 w-full py-2 bg-zinc-900 border border-zinc-700 hover:bg-zinc-800 rounded-lg text-sm font-semibold transition-all text-zinc-300">
            <DownloadCloud size={16} /> Import Chat
          </button>
          <button className="flex items-center justify-center gap-2 w-full py-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-sm font-semibold transition-all">
      ...
      <main className="flex-1 flex flex-col relative min-w-0">
        <header className="h-16 border-b border-zinc-800 flex items-center justify-between px-6 bg-zinc-950/80 backdrop-blur-sm z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleMerge} className="flex items-center gap-2 px-4 py-1.5 rounded-lg border border-zinc-700 hover:bg-zinc-900 text-xs font-medium transition-all text-zinc-300">
            ...
            messages.map((m, i) => (
              <div key={i} className={`max-w-3xl mx-auto flex gap-4 ${m.role === 'user' ? 'justify-end' : m.role === 'system' ? 'justify-center' : 'justify-start'}`}>
                <div className={`p-4 rounded-2xl max-w-[85%] shadow-sm relative group ${
                  ...
                  : m.role === 'system'
                  ? 'bg-zinc-900/50 border border-zinc-800 text-zinc-400 rounded-lg w-full text-center text-xs font-mono tracking-wide'
                  ...
                  {m.id && m.id !== 'temp' && m.role !== 'system' && (
                    <button
                      onClick={() => setForkModal({ isOpen: true, messageId: m.id, name: "", isEphemeral: true })}
        ...
        <div className="p-6 pt-2 bg-gradient-to-t from-zinc-950 via-zinc-950 to-transparent">
          <div className="max-w-3xl mx-auto relative group flex items-center gap-2">
            ...
            <div className="relative flex-1">
              ...
              />
              <button onClick={handleSend} disabled={loading || switching || !input.trim()} className="absolute right-2 top-1/2 -translate-y-1/2 p-2.5 bg-indigo-600 rounded-xl hover:bg-indigo-500 disabled:opacity-50 disabled:bg-zinc-800 transition-all active:scale-95">
